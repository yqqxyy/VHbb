 ...
 *
 * This file contains source code for the class StochasticLib2 defined in
 *stocc.h.
 *
 * Documentation:
 * ==============
...
#include "CxAODReader/stocc.h"  // class definition
...
int32_t StochasticLib2::BinomialModeSearch(int32_t n, double p) {
  ...
  */
  ...
  if (n != bino_n_last || p != bino_p_last) {
    ...
    bino_r1 = p / (1.0 - p);
    bino_g = exp(LnFac(n) - LnFac(bino_mode) - LnFac(n - bino_mode) +
                 bino_mode * log(p) + (n - bino_mode) * log(1. - p));
  }
  while (1) {
    U = Random();
  ...
  */
  ...
  for (;;) {
    ...
    if ((U = Random() * Bino_p6) < Bino_p2) {  // centre left
      ...
      if ((W = V / Bino_dl) < Bino_f1)
        return (Bino_k1 + (int32_t)(V / Bino_f1));

      // computation of candidate X < k2, and its counterpart Y > k2
      // either squeeze-acceptance of X or acceptance-rejection of Y
      Dk = (int32_t)(Bino_dl * Random()) + 1;
      if (W <=
          Bino_f2 - Dk * (Bino_f2 - Bino_f2 / Bino_r2)) {  // quick accept of
        return (Bino_k2 - Dk);
      }                                        // X = k2 - Dk
      if ((V = Bino_f2 + Bino_f2 - W) < 1.) {  // quick reject of Y
        Y = Bino_k2 + Dk;
        if (V <= Bino_f2 +
                     Dk * (1. - Bino_f2) / (Bino_dl + 1.)) {  // quick accept of
          return (Y);
        }  // Y = k2 + Dk
        if (V <= BinomialF(Y, n, Bino_l_pq, Bino_c_pm)) return (Y);
      }  // final accept of Y
      X = Bino_k2 - Dk;
    }
      ...
      if ((V = U - Bino_p3) < 0.)
        return (Bino_k4 - (int32_t)((U - Bino_p2) / Bino_f4));
      // immediate acceptance region R4 = [k4+1, k5+1)*[0, f5)
      if ((W = V / Bino_dr) < Bino_f5)
        return (Bino_k5 - (int32_t)(V / Bino_f5));

      // computation of candidate X > k4, and its counterpart Y < k4
      // either squeeze-acceptance of X or acceptance-rejection of Y
      Dk = (int32_t)(Bino_dr * Random()) + 1;
      if (W <=
          Bino_f4 - Dk * (Bino_f4 - Bino_f4 * Bino_r4)) {  // quick accept of
        return (Bino_k4 + Dk);
      }                                        // X = k4 + Dk
      if ((V = Bino_f4 + Bino_f4 - W) < 1.) {  // quick reject of Y
        ...
        if (V <= BinomialF(Y, n, Bino_l_pq, Bino_c_pm))
...
double StochasticLib2::BinomialF(int32_t k, int32_t n, double l_pq,
                                 double c_pm) {
...
int32_t StochasticLib2::PoissonModeSearch(double L) {
  ...
  */
  ...
  if (L != pois_L_last) {  // set up
    pois_L_last = L;
    pois_bound = (int32_t)floor(
        L + 0.5 + 7.0 * (sqrt(L + L + 1.) + 1.5));   // safety-bound
  ...
  */
  ...
  for (;;) {
    ...
    if ((U = Random() * Pois_p6) < Pois_p2) {  // centre left
      ...
      // immediate acceptance region R1 = [k1, k2)*[0, f1),  X = k1, ... k2-1
      if ((W = V / Pois_dl) < Pois_f1)
        return (Pois_k1 + (int32_t)(V / Pois_f1));

      // computation of candidate X < k2, and its counterpart Y > k2
      // either squeeze-acceptance of X or acceptance-rejection of Y
      Dk = (int32_t)(Pois_dl * Random()) + 1;
      if (W <=
          Pois_f2 - Dk * (Pois_f2 - Pois_f2 / Pois_r2)) {  // quick accept of
        return (Pois_k2 - Dk);                             // X = k2 - Dk
      }

      if ((V = Pois_f2 + Pois_f2 - W) < 1.0) {  // quick reject of Y
        Y = Pois_k2 + Dk;
        if (V <= Pois_f2 + Dk * (1.0 - Pois_f2) /
                               (Pois_dl + 1.0)) {  // quick accept of
          return Y;                                // Y = k2 + Dk
        }
        if (V <= PoissonF(Y, Pois_l_my, Pois_c_pm))
          return Y;  // final accept of Y
      }
      X = Pois_k2 - Dk;
      ...
      //  immediate acceptance region R3 = [mode, k4+1)*[0, f4), X = mode, ... k4
      if ((V = U - Pois_p3) < 0.)
        return (Pois_k4 - (int32_t)((U - Pois_p2) / Pois_f4));
      // immediate acceptance region R4 = [k4+1, k5+1)*[0, f5)
      if ((W = V / Pois_dr) < Pois_f5)
        return (Pois_k5 - (int32_t)(V / Pois_f5));

      // computation of candidate X > k4, and its counterpart Y < k4
      // either squeeze-acceptance of X or acceptance-rejection of Y
      Dk = (int32_t)(Pois_dr * Random()) + 1L;
      if (W <=
          Pois_f4 - Dk * (Pois_f4 - Pois_f4 * Pois_r4)) {  // quick accept of
      ...
      if (U < Pois_p5) {  // expon. tail left
        ...
        if (W <= Pois_f1 - Dk * (Pois_f1 - Pois_f1 / Pois_r1))
          return X;  // quick accept of X
      } else {       // expon. tail right
        Dk = (int32_t)(1.0 - log(W) / Pois_lr);
        X = Pois_k5 + Dk;              // X >= k5 + 1
        W *= (U - Pois_p5) * Pois_lr;  // W -- U(0, h(x))
        if (W <= Pois_f5 - Dk * (Pois_f5 - Pois_f5 * Pois_r5))
          return X;  // quick accept of X
      }
    }
...
int32_t StochasticLib2::HypPatchwork(int32_t n, int32_t m, int32_t N) {
  ...
  */
  ...
  if (N != hyp_N_last || m != hyp_m_last || n != hyp_n_last) {
    ...
    // recurrence constants r(k) = p(k)/p(k-1) at k = k1, k2, k4+1, k5+1
    Hyp_r1 =
        (np / (double)Hyp_k1 - 1.) * (Mp - Hyp_k1) / (double)(Hyp_L + Hyp_k1);
    Hyp_r2 =
        (np / (double)Hyp_k2 - 1.) * (Mp - Hyp_k2) / (double)(Hyp_L + Hyp_k2);
    Hyp_r4 = (np / (double)(Hyp_k4 + 1) - 1.) * (m - Hyp_k4) /
             (double)(Hyp_L + Hyp_k4 + 1);
    Hyp_r5 = (np / (double)(Hyp_k5 + 1) - 1.) * (m - Hyp_k5) /
             (double)(Hyp_L + Hyp_k5 + 1);
  ...
  while (1) {
    ...
    if ((U = Random() * Hyp_p6) < Hyp_p2) {  // centre left
      ...
      if ((W = Hyp_f2 + Hyp_f2 - Y) < 1.) {  // quick reject of V
        V = Hyp_k2 + Dk;
        if (W <=
            Hyp_f2 + Dk * (1. - Hyp_f2) / (Hyp_dl + 1.)) {  // quick accept of V
          return (V);
        }
      ...
      // immediate acceptance region R3 = [mode, k4+1)*[0, f4), X = mode, ... k4
      if ((W = U - Hyp_p3) < 0.)
        return (Hyp_k4 - (int32_t)((U - Hyp_p2) / Hyp_f4));
