 ...
 *
 * This file contains source code for the class StochasticLib1 defined in
 *stocc.h.
 *
 * Documentation:
 * ==============
...
Constructor
***********************************************************************/
StochasticLib1::StochasticLib1(int seed) : STOC_BASE(seed) {
  // Initialize variables for various distributions
  normal_x2_valid = 0;
  hyp_n_last = hyp_m_last = hyp_N_last =
      -1;             // Last values of hypergeometric parameters
  pois_L_last = -1.;  // Last values of Poisson parameters
  bino_n_last = -1;
  bino_p_last = -1.;  // Last values of binomial parameters
}
  ...
  */
  ...
  if (N != hyp_N_last || m != hyp_m_last || n != hyp_n_last) {
    ...
    hyp_fm = exp(LnFac(N - m) - LnFac(L + hyp_mode) - LnFac(n - hyp_mode) +
                 LnFac(m) - LnFac(m - hyp_mode) - LnFac(hyp_mode) - LnFac(N) +
                 LnFac(N - n) + LnFac(n));
...
int32_t StochasticLib1::HypRatioOfUnifoms(int32_t n, int32_t m, int32_t N) {
  ...
  */
  ...
  if (hyp_N_last != N || hyp_m_last != m || hyp_n_last != n) {
    ...
    mode = (int32_t)(double(n + 1) * double(m + 1) * rNN *
                     N);  // mode = floor((n+1)*(m+1)/(N+2))
    var = (double)n * m * (N - m) * (N - n) /
          ((double)N * N * (N - 1));             // variance
...
***********************************************************************/
void StochasticLib1::MultiHypergeometric(int32_t* destination, int32_t* source,
...
int32_t StochasticLib1::BinomialRatioOfUniforms(int32_t n, double p) {
  ...
  */
  ...
  while (1) {  // rejection loop
    ...
    lf = (k - bino_mode) * bino_r1 + bino_g - LnFac(k) -
         LnFac(n - k);                     // ln(f(k))
    if (u * (4.0 - u) - 3.0 <= lf) break;  // lower squeeze accept
    if (u * (u - lf) > 1.0) continue;      // upper squeeze reject
    if (2.0 * log(u) <= lf) break;         // final acceptance
...
***********************************************************************/
void StochasticLib1::Multinomial(int32_t* destination, double* source,
                                 int32_t n, int colors) {
  /*
  This function generates a vector of random variates, each with the
  binomial distribution.
...
double StochasticLib1::Normal(double m, double s) {
  ...
  normal_x1 *= w;
  normal_x2 *=
      w;  // normal_x1 and normal_x2 are independent normally distributed variates
