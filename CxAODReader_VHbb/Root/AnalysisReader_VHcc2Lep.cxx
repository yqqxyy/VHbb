...
AnalysisReader_VHcc2Lep::AnalysisReader_VHcc2Lep() : AnalysisReader_VHQQ2Lep() {
  m_doMergeModel = false;
}

AnalysisReader_VHcc2Lep::~AnalysisReader_VHcc2Lep() {}

EL::StatusCode AnalysisReader_VHcc2Lep::run_2Lep_analysis() {
  ...
  if ((unsigned int)m_physicsMeta.nAddBTrkJets !=
    ...
    Error("AnalysisReader_VHcc2Lep::run_2Lep_analysis",
  ...
  EL_CHECK("AnalysisReader_VHcc2Lep::run_2Lep_analysis()",
  ...
  if (m_doKF) {
    if (m_analysisStrategy == "Resolved") {
      ...
      if ((m_physicsMeta.flavor == PhysicsMetadata::Flavor::ElEl ||
          ...
          (m_physicsMeta.nTags == 2 || m_physicsMeta.nTags == 1)) {
  ...
  //----------------------------
  if (isMu || isE) updateFlag(eventFlag, DiLeptonCuts::SFLeptons);
  ...
  if (ZVec.Pt() / 1.e3 > 200) {
    dRCut = 1.3;
  } else if (ZVec.Pt() / 1.e3 > 150) {
    dRCut = 1.5;
  } else if (ZVec.Pt() / 1.e3 > 75) {
    dRCut = 2.2;
  }

  if (m_model == Model::CUT && selectedJets.size() > 1 &&
      j1.vec_corr.DeltaR(j2.vec_corr) < dRCut) {
    updateFlag(eventFlag, DiLeptonCuts::dRCut);
  }

  // **Selection**  : nSubleadingBtags == 0 (VHcc analysis strategy: veto on 3+
  // jets)
  if (nSubleadingBtags == 0) {
    updateFlag(eventFlag, DiLeptonCuts::Veto3pJets);
  }
  ...
  if (m_model == Model::CUT) {
    cuts_common_resolved = {
        ...
        DiLeptonCuts::OSLeptons,       DiLeptonCuts::AtLeast2Jets,
        DiLeptonCuts::AtLeast2SigJets, DiLeptonCuts::PtB145,
        DiLeptonCuts::Veto3pJets,      DiLeptonCuts::dRCut};
  ...
  /////////////////////////////////////////////////////////////
  //**Emulate C-tag norm syst**
  /*
  if (!nominalOnly && isNominal && m_isMC) {
    // define jets used for c-tagging
    std::vector<const xAOD::Jet *> jetsForBTagging;
    if (m_tagStrategy == "AllSignalJets")
      jetsForBTagging = signalJets;
    else if (m_tagStrategy == "Leading2SignalJets") {
      if (selectedJets.size() > 1)
        jetsForBTagging = {
            selectedJets.at(0),
            selectedJets.at(1)};  // if there are not at leas 2 we don't care
                                  // about the syst...
    } else {
      Error("run_2Lep_analysis()",
            "Trying to run c-tag syst emulation for tag strategy "
            "LeadingSignalJets; not provided!");
      return EL::StatusCode::FAILURE;
    }
    emulateCTagSystematics(jetsForBTagging);
  }
  */
  /////////////////////////////////////////////////////////////
  ...
  if (m_physicsMeta.nSigJet >= 2 || fatJets.size() >= 1) {
    ...
    // VHcc analysis strategy variables
    m_etree->SetBranchAndValue<float>("nSubleadingBtags", nSubleadingBtags,
                                      -99);
    m_etree->SetBranchAndValue<float>("btagWeight", btagWeight, -99);

    /*
    // here
    m_histSvc->BookFillHist("nSubleadingBtags", 5, 0, 5, nSubleadingBtags,
    m_weight); m_histSvc->BookFillHist("bTagWeight", 20, 0, 1, btagWeight,
    */
    ...
    if (m_isMC && m_histNameSvc->get_description() != "") {
      for (int n_truthtags = 0; n_truthtags < 3; n_truthtags++) {
        ...
        m_histSvc->BookFillHist("mBBTT", 100, 0, 500,
                                m_etree->get<float>("mBB"), weight);
        /*
        m_histSvc->BookFillHist("EtaB1TT", 60, -3, 3,
                                m_etree->get<float>("etaB1"), weight);
        m_histSvc->BookFillHist("EtaB2TT", 60, -3, 3,
                                m_etree->get<float>("etaB2"), weight);
        m_histSvc->BookFillHist("pTVTT", 100, 0, 1000,
                                m_etree->get<float>("pTV"), weight);
        m_histSvc->BookFillHist("pTB1TT", 200, 0, 2000,
                                m_etree->get<float>("pTB1"), weight);
        m_histSvc->BookFillHist("pTB2TT", 150, 0, 1500,
                                m_etree->get<float>("pTB2"), weight);
        m_histSvc->BookFillHist("dRBBTT", 100, 0, 5,
                                m_etree->get<float>("dRBB"), weight);
        */
        if (do_btag_Syst) {
      ...
      m_histSvc->BookFillHist("mBBTT", 100, 0, 500, m_etree->get<float>("mBB"),
                              m_weight);
      /*
      m_histSvc->BookFillHist("EtaB1TT", 60, -3, 3,
                              m_etree->get<float>("etaB1"), m_weight);
      m_histSvc->BookFillHist("EtaB2TT", 60, -3, 3,
                              m_etree->get<float>("etaB2"), m_weight);
      m_histSvc->BookFillHist("pTVTT", 100, 0, 1000, m_etree->get<float>("pTV"),
                              m_weight);
      m_histSvc->BookFillHist("pTB1TT", 200, 0, 2000,
                              m_etree->get<float>("pTB1"), m_weight);
       m_histSvc->BookFillHist("pTB2TT", 150, 0, 1500,
                               m_etree->get<float>("pTB2"), m_weight);
       m_histSvc->BookFillHist("dRBBTT", 100, 0, 5, m_etree->get<float>("dRBB"),
                              m_weight);
       */
    }
    ...
    if (m_isMC) {
      m_etree->SetBranchAndValue<float>("TruthTaggingEventWeight_dt",
                                        btag_TT_weights["Nominal"][-1], -99);
      m_etree->SetBranchAndValue<float>(
          "TruthTaggingEventWeight_0c",
          eventweight_before_btagging_weight * btag_TT_weights["Nominal"][0],
          -99);
      m_etree->SetBranchAndValue<float>(
          "TruthTaggingEventWeight_1c",
          eventweight_before_btagging_weight * btag_TT_weights["Nominal"][1],
          -99);
      m_etree->SetBranchAndValue<float>(
          "TruthTaggingEventWeight_2c",
          eventweight_before_btagging_weight * btag_TT_weights["Nominal"][2],
          -99);
      m_etree->SetBranchAndValue<float>(
          "TruthTaggingEventWeight_Leading_c",
          eventweight_before_btagging_weight * btag_TT_weights["Nominal"][4],
          -99);
      m_etree->SetBranchAndValue<float>(
          "TruthTaggingEventWeight_SubLeading_c",
          eventweight_before_btagging_weight * btag_TT_weights["Nominal"][5],
          -99);
    } else {
      m_etree->SetBranchAndValue<float>("TruthTaggingEventWeight_dt", 1, -99);
      m_etree->SetBranchAndValue<float>("TruthTaggingEventWeight_0c", 1, -99);
      m_etree->SetBranchAndValue<float>("TruthTaggingEventWeight_1c", 1, -99);
      m_etree->SetBranchAndValue<float>("TruthTaggingEventWeight_2c", 1, -99);
      m_etree->SetBranchAndValue<float>("TruthTaggingEventWeight_Leading_c", 1,
                                        -99);
      m_etree->SetBranchAndValue<float>("TruthTaggingEventWeight_SubLeading_c",
                                        1, -99);
    }
    ...
    if (writeObjectsInEasyTree && !writeEasyTree) writeEasyTree = true;
  ...
  // Info("AnalysisReader_VHcc2Lep::run_2Lep_analysis()", "Region: %s, and
...
EL::StatusCode AnalysisReader_VHcc2Lep::fill_2lepCutFlow(
  ...
  // Resolved cutlfow
  static std::string cuts[22] = {
      "All",         "LeadLepPt27", "Trigger",      "2LepSF",
      "OSleptons",   "mll",         "AtLeast2Jets", "AtLeast2SigJets",
      "pTB145",      "Veto3pJets",  "0tag_23jets",  "0tag_4jets",
      "0tag_5pjets", "1tag_23jets", "1tag_4jets",   "1tag_5pjets",
      "2tag_23jets", "2tag_4jets",  "2tag_5pjets",  "3ptag_23jets",
      "3ptag_4jets", "3ptag_5pjets"};
  ...
  std::vector<unsigned long int> cutsResolved = {
      ...
      DiLeptonCuts::AtLeast2Jets, DiLeptonCuts::AtLeast2SigJets,
      DiLeptonCuts::PtB145,       DiLeptonCuts::Veto3pJets};
  ...
  // Fill rest of bins based on ntags, njets, only for events that pass all cuts
  // in cutsResolved
  ...
  // Merged cutflow
  static std::string cutsMergedNames[14] = {
      "All",        "Trigger",    "2LepSF",     "OSleptons",      "PassEtaCuts",
      "mll",        "AtLeast1FJ", "mbbCorr",    "AtLeast2TJinFJ", "PtZGT500",
      "0tag_1pfat", "1tag_1pfat", "2tag_1pfat", "3ptag_1pfat"};

  std::vector<unsigned long int> cutsMerged = {
      DiLeptonCuts::AllCxAOD,     DiLeptonCuts::Trigger,
      DiLeptonCuts::SFLeptons,    DiLeptonCuts::OSLeptons,
      DiLeptonCuts::MuonEtaLT2p5, DiLeptonCuts::MllZwindowMerged,
      DiLeptonCuts::AtLeast1FJ,   DiLeptonCuts::mHCorrMerged,
      DiLeptonCuts::LeadFJ2TJ,    DiLeptonCuts::PtZGT500};
  incrementCuts.clear();
  ...
  for (unsigned long int i = 0; i < cutsMerged.size(); ++i) {
    ...
    m_histSvc->BookFillCutHist(dir + "CutsMerged", length(cutsMergedNames),
                               cutsMergedNames, label, m_weight);
    m_histSvc->BookFillCutHist(dir + "CutsMergedNoWeight",
                               length(cutsMergedNames), cutsMergedNames, label,
                               1.);
  }
